...
let table_elements =[]
let selectedElements = {element:null}
    ...
    table_elements.forEach(element => {
      ...
      element.row.addEventListener("click", () =>{
        ...
        if (element.type == true) {
          selectedElements.element = element["location"]
            table_elements.forEach(element1 =>{     
              element1.row.remove()
            })
...
function CreateTableElement(name,date,ext,size,type,absPath){
    ...
    table_elements.push({row:Frow,type:type,location:absPath})
  ...
  function addElement(name){
    ...
    const target = document.querySelector(".se-elements")
